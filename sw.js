const CACHE='football-coach-v2.1.0';
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith(caches.match(event.request).then(hit=>hit||fetch(event.request).then(resp=>{
    if(new URL(event.request.url).origin===location.origin){const copy=resp.clone();caches.open(CACHE).then(c=>c.put(event.request,copy));}
    return resp;
